...
use lurk::coprocessor::sha3::{Sha3Coproc, Sha3Coprocessor};
use lurk::{
...
fn sha3_nivc<F: LurkField>(store: &Store<F>, n: usize, input: &[usize]) -> Ptr {
...
fn main() {
    ...
    let cproc_sym = user_sym(&format!("sha3_nivc_{n}"));

    let call = sha3_nivc(store, n, &(0..n).collect::<Vec<_>>());

    let mut lang = Lang::<Fr, Sha3Coproc<Fr>>::new();
    lang.add_coprocessor(cproc_sym, Sha3Coprocessor::new(n));
    ...
    let supernova_prover = SuperNovaProver::<Fr, Sha3Coproc<Fr>, MultiFrame<'_, _, _>>::new(
        REDUCTION_COUNT,
        lang_rc.clone(),
    );
